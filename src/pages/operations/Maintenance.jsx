...
export default function Maintenance({user}) {
  ...
  const columns=[{key:"droneCode",label:"Drone"},{key:"type",label:"Work",render:r=><button className="link-button strong-link" type="button" onClick={()=>edit(r)}>{r.type}</button>},{key:"displayStatus",label:"Status",filterable:true,render:r=><StatusBadge>{r.displayStatus}</StatusBadge>},{key:"due",label:"Due"},{key:"completed",label:"Last serviced",render:r=>r.completed}] ;
  ...
  return <div className="page-stack operations-module">
    ...
    <div className="panel"><SectionHeader title="Service Schedule"/><DataTable rows={drones} getRowKey={d=>d.id} columns={[{key:"droneCode",label:"Drone"},{key:"status",label:"Status",render:d=><StatusBadge>{d.maintenanceOverdue?"OVERDUE":d.status}</StatusBadge>},{key:"flightHours",label:"Flight Hours"},{key:"lastServicedDate",label:"Last serviced",render:d=>d.lastServicedDate?.slice(0,10)||"Not recorded"},{key:"nextMaintenanceDate",label:"Next inspection",render:d=>d.nextMaintenanceDate?.slice(0,10)||"Not scheduled"},{key:"certificationStatus",label:"Certification",render:d=><StatusBadge>{d.certificationStatus}</StatusBadge>}]}/></div>
    {editing&&<div className="modal-backdrop"><form className="modal-dialog registration-dialog maintenance-form-dialog" role="dialog" aria-modal="true" aria-label="Maintenance record" onSubmit={save}><div className="modal-header"><div><p className="eyebrow">Aircraft Maintenance</p><h2>{id?"Maintenance record":"Schedule maintenance"}</h2></div><button className="icon-button danger" type="button" disabled={busy} onClick={()=>setEditing(false)} aria-label="Close maintenance record" title="Close"><X size={18}/></button></div><div className="modal-body"><fieldset className="operations-form form-grid" disabled={!canManage||closed||busy}>
      ...
      </fieldset></div><div className="modal-footer">{canManage&&!closed&&<ActionButton variant="primary" icon={Save} disabled={busy} isLoading={busy} type="submit">Save maintenance</ActionButton>}</div></form></div>}
  </div>;
}
