...
export default function Pilots({user}) {
  ...
  return <div className="page-stack operations-module">
    ...
    {selected&&<div className="modal-backdrop"><div className="modal-dialog registration-dialog pilot-profile-dialog" role="dialog" aria-modal="true" aria-label="Pilot profile"><div className="modal-header pilot-profile-header"><div><p className="eyebrow">Pilot Profile</p><h2>{selected.name}</h2><p>{selected.email}</p></div><div className="pilot-profile-actions">{canManage&&<ActionButton icon={editing?X:null} onClick={()=>editing?cancelEdit():setEditing(true)}>{editing?"Cancel":"Edit credentials"}</ActionButton>}<ActionButton onClick={()=>setSelected(null)}>Close</ActionButton></div></div><div className="modal-body"><div className="pilot-profile-grid">
